...
from hypyp.sync.accorr import (
    _accorr_hybrid, 
    _accorr_hybrid_vectorized, 
    _accorr_hybrid_precompute, 
    _accorr_hybrid_precompute_numba,
    _accorr_hybrid_numba,
    _accorr_hybrid_precompute_torch,
    _accorr_hybrid_precompute_torch_loop,
    NUMBA_AVAILABLE
)
import numba
...
preproc_S1 = mne.read_epochs('preproc_S1.fif')
preproc_S2 = mne.read_epochs('preproc_S2.fif')
...
def numba_run(cpu_num, numba_f):
    def f(complex_signal: np.ndarray, 
          epochs_average: bool = True, 
          show_progress: bool = True):
          numba.set_num_threads(cpu_num)
          return numba_f(complex_signal, epochs_average, show_progress)
    return f


def torch_run(device, torch_f):
    def f(complex_signal: np.ndarray, 
          epochs_average: bool = True, 
          show_progress: bool = True):
          return torch_f(complex_signal, epochs_average, 
                         show_progress, device=device)
    return f


method_dict = {
    'original': _accorr_hybrid,
    'numba4': numba_run(4, _accorr_hybrid_numba),
    'numba8': numba_run(8, _accorr_hybrid_numba),
    'numba_precompute8': numba_run(8, _accorr_hybrid_precompute_numba),
    'precompute': _accorr_hybrid_precompute,
    'torch_precompute_mps': torch_run('mps', _accorr_hybrid_precompute_torch),
    ...
    'torch_loop_cpu': torch_run('cpu', _accorr_hybrid_precompute_torch_loop),
}

numba_palette = sns.light_palette('C1', 3)
numba_precompute_palette = sns.light_palette('C4', 3)
torch_palette = sns.light_palette('C5', 4)
torch_loop_palette = sns.light_palette('C6', 4)
method_palette = OrderedDict({
    ...
    'vectorized': 'C2',
    'precompute': 'C3',
    'numba_precompute8': numba_precompute_palette[2],
    'torch_precompute_cpu': torch_palette[1],
    'torch_precompute_mps': torch_palette[2],
    'torch_precompute_cuda': torch_palette[3],
    'torch_loop_cpu': torch_loop_palette[1],
    'torch_loop_mps': torch_loop_palette[2],
})

out_path = Path('accorr_benchmarks')
...
def benchmark(method, epoch_multiplier, channel_multiplier):
    ...
    try:
        ...
        result = analyses.compute_sync(complex_signal, mode=method_dict[method], 
        ...
        max_diff = None
        if method != 'original':
            orig_path = out_path / f'original-e{epoch_multiplier}-c{channel_multiplier}_result.pkl'
...
def task_calc_benchmarks():
    ...
    for _, r in benchmark_configs.iterrows():
        for method in method_dict:
            ...
            orig_name = f'original-e{r["epoch_multiplier"]}-c{r["channel_multiplier"]}'
