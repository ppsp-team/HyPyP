...
def transform(locs: np.ndarray, traX: float=0.15, traY: float=0, traZ: float=0.5, rotY: float=(np.pi)/2, rotZ: float=(np.pi)/2, children: bool=False, child_head: bool=False) -> np.ndarray:
    ...
    child_head (bool, optional): If True, apply transformations specific to a child's head model. Only relevant if `children` is True. Default is False.
    Returns:
...
def transform_2d_intra(locs: np.ndarray, traX: float=0.15, traY: float=0, traZ: float=0, rotY: float=(np.pi)/2, rotZ: float=(np.pi)/2, children: bool=False, child_head: bool=False) -> np.ndarray:
    ...
    if children:
        if child_head:
            # Transformations for a child's head model (from transform_petit)
            scale = 0.68
        else:
            # Transformations for an adult's head model (from transform_grand)
            scale = 1.03
    else:
        scale = 1.0
...
def plot_sensors_3d_intra(ax: str, epo1: mne.Epochs, epo2: mne.Epochs, lab: bool = False, children : bool = False, child_head:bool = False ):
    ...
    if children : 
        ...
        loc1 = transform(loc1, traX=0, traY=0, traZ=0.04, rotY=0, rotZ=(-np.pi/2), children=children)
        loc2 = transform(loc2, traX=0, traY=0.5, traZ=0.04, rotY=0, rotZ=(-np.pi/2), children=children)
...
def plot_links_2d_inter(epo1: mne.Epochs, epo2: mne.Epochs, C: np.ndarray, threshold: str='auto', steps: int=10, children: bool=False, child_head: bool=False):
    ...
    def bezier_interpolation(t, p0, p1, c0, c1):
        return ((1 - t) ** 3 * p0 +
            3 * (1 - t) ** 2 * t * (2 * p0 - c0) +
            3 * (1 - t) * t ** 2 * (2 * p1 - c1) +
            t ** 3 * p1)
...
def plot_links_2d_intra(epo1: mne.Epochs, epo2: mne.Epochs, C1: np.ndarray, C2: np.ndarray, threshold: str='auto', steps: int=10, children: bool=False, child_head: bool=False):
    ...
    Returns:
        None: plot the links in 2D within the current axis.
    """
    def bezier_interpolation(t, p0, p1, c):
        return ((1 - t) ** 3 * p0 +
            3 * (1 - t) ** 2 * t * (2 * p0 - c) +
            3 * (1 - t) * t ** 2 * (2 * p1 - c) +
            t ** 3 * p1)
    ...
    def plot_bezier_curve(x1, y1, x2, y2, ctr, color, weight, steps):
        if steps <= 2:
            ...
            for idx in range(len(alphas) - 1):
                ...
                ynn = bezier_interpolation(b, y1, y2, ctr1[1])
                plt.plot([xn, xnn], [yn, ynn], '-', color=color, linewidth=weight)
...
def plot_links_3d_intra(ax: str, epo1: mne.Epochs, epo2: mne.Epochs,C1: np.ndarray, C2: np.ndarray, threshold: str='auto', steps: int=10, children: bool=False, child_head: bool=False):
    ...
    if children: 
        loc2 = transform_2d_intra(loc2, traX=0, traY=0.5, traZ=0.04, rotY=0, rotZ=(-np.pi/2), children=children, child_head=child_head)
        loc1 = transform_2d_intra(loc1, traX=0, traY=0, traZ=0.04, rotY=0, rotZ=(-np.pi/2), children=children, child_head=not child_head)
    # extract sensor infos and transform loc to fit with headmodel 
    else :  
...
def viz_2D_headmodel_inter (epo1: mne.Epochs, epo2: mne.Epochs, C: np.ndarray, threshold: float=0.95, steps: int=10, lab: bool = True, children: bool=False):
    """
    ...
    if children:
        vertices, faces = get_3d_heads_inter(children=True)
    else:
    ...
    if children : 
        plot_sensors_2d_inter(epo1, epo2, lab=lab, children=True)
        plot_links_2d_inter(epo1, epo2, C=C, threshold=threshold, steps=steps, children=True)
    else : 
...
def viz_2D_topomap_inter(epo1: mne.Epochs, epo2: mne.Epochs, C: np.ndarray, threshold: float=0.95, steps: int=10, lab: bool = False, children: bool=False):
    """
...
def viz_2D_topomap_intra (epo1: mne.Epochs, epo2: mne.Epochs, C1: np.ndarray, C2: np.ndarray, threshold: float=0.95, steps: int=2, lab: bool = False, children : bool = False):
    """
    ...
    if children : 
        plot_sensors_2d_intra(epo1, epo2, lab = lab, children = True)
        ...
        plot_links_2d_intra(epo1, epo2, C1=C1, C2=C2, threshold=threshold, steps=steps, children = True)
    else : 
...
def viz_3D_inter (epo1: mne.Epochs, epo2: mne.Epochs, C: np.ndarray, threshold: float=0.95, steps: int=10, lab: bool = False, children: bool = False):
    ...
    if children : 
        plot_sensors_3d_inter(ax, epo1, epo2, lab=lab, children=True) 
        plot_links_3d_inter(ax, epo1, epo2, C=C, threshold=threshold, steps=steps, children=True)
    else : 
...
def viz_3D_intra (epo1: mne.Epochs, epo2: mne.Epochs, C1: np.ndarray, C2: np.ndarray, threshold: float=0.95, steps: int=10, lab: bool = False, children :bool = False):
    """
    ...
    # defining head model and adding sensors
    if children : 
        vertices, faces = get_3d_heads_intra(children=True)
    else : 
    ...
    if children : 
        # bads are represented as squares
        plot_sensors_3d_intra(ax, epo1, epo2, lab=lab, children = True)
        # plotting links according to sign (red for positive values,
        # blue for negative) and value (line thickness increases
        # with the strength of connectivity)
        plot_links_3d_intra(ax, epo1, epo2, C1=C1, C2=C2, threshold=threshold, steps=steps, children=  True)
    else: 
