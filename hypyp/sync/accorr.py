    ...
    if optimization == None:
        ...
        if optimization == TORCH_MPS_OPTIMIZATION:
            if MPS_AVAILABLE:
                ...
                raise ValueError('MCP not available on this device for the selected optimization')
...
if NUMBA_AVAILABLE:
    ...
    def _accorr_den_calc_precalc(n_epochs : int, n_freq : int, n_ch_total : int, 
        ...
        """
        den = np.zeros((n_epochs, n_freq, n_ch_total, n_ch_total))
        print((n_epochs, n_freq, n_ch_total, n_ch_total))
        print(angle.shape)
        print(n_adj_all.shape)
        print(m_adj_all.shape)
